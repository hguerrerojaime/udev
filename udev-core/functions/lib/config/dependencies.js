...
module.exports = {
    ...
    factories: {
        regionDAOFactory: (context) => {
            ...
            return (realmId) => {
                return new RegionDAO(context.container.get('db'), context.container.get('recordAuditor'), context.container.get('realmDAO'), realmId);
            };
        },
        userRegionDAOFactory: (context) => {
            const UserRegionDAO = require('../dao/UserRegionDAO').default;
            return (realmId) => {
                return new UserRegionDAO(context.container.get('db'), context.container.get('recordAuditor'), context.container.get('realmDAO'), realmId);
            };
        },
        modelDAOFactory: (context) => {
            const ModelDAO = require('../dao/ModelDAO').default;
            return (realmId, regionId) => {
                return new ModelDAO(context.container.get('db'), context.container.get('recordAuditor'), context.container.get('regionDAOFactory'), realmId, regionId);
            };
        }
    }
};
